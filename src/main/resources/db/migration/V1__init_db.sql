    ...
    action_type varchar(50)  not null
        constraint audit_log_action_type_check
            check ((action_type)::text = ANY
                   ((ARRAY ['GET'::character varying, 'GET_ALL'::character varying, 'CREATE'::character varying, 'BULK_CREATE'::character varying, 'UPDATE'::character varying, 'BULK_UPDATE'::character varying, 'DELETE'::character varying, 'BULK_DELETE'::character varying, 'STATUS_CHANGE'::character varying])::text[])),
    entity_name varchar(100) not null
        constraint audit_log_entity_name_check
            check ((entity_name)::text = ANY
                   ((ARRAY ['USER'::character varying, 'ROLE'::character varying, 'LEAD'::character varying, 'CONTACT'::character varying, 'ACTIVITY'::character varying, 'ACTIVITY_TYPE'::character varying, 'REPORT'::character varying])::text[])),
...
(
    ...
    assigned_user_id   bigint       not null
        constraint fk8h0qwisrf9205mj84mf1vl33m
            references users,
    contact_id         bigint
